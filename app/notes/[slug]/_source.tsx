...
function NotePage() {
  const { note, body } = undefined as any;
  return (
    <main className="min-h-screen bg-background text-foreground">
      <article className="mx-auto max-w-2xl px-5 pb-24 pt-24 sm:px-8 sm:pt-32">
        <Link href="/notes" className="inline-flex items-center gap-2 font-mono text-[11px] uppercase tracking-widest text-muted-foreground hover:text-foreground transition-colors">
        ...
        <div className="mt-3"><ViewCounter storageKey={`note:${note.slug}`} label="reads this week" /></div>
        <h1 className="font-display mt-3 text-3xl font-bold leading-[1.1] tracking-tight sm:text-5xl">{note.title}</h1>
